...
# 【修复】缓冲区大小（最关键！从 64KB 提升到 32MB 加上就无法启动SSHD，导致无法连接。要用web termerinal）
# 访问holefrog.dynamic-dns.net会直到显示aria2下载界面
...
${SFTP_MARKER}
# ============================================
# SFTP Configuration for $FTP_USER
# 自动生成于 $(date '+%Y-%m-%d %H:%M:%S')
# ============================================
Match User $FTP_USER
    # ChrootDirectory: 将用户锁定在此目录
    ChrootDirectory $CHROOT_DIR
    
    # ForceCommand: 强制使用 SFTP，禁止 Shell
    ForceCommand internal-sftp
    
    # PasswordAuthentication: 允许密码登录
    PasswordAuthentication yes
    
    # X11Forwarding: 禁止 X11 转发
    X11Forwarding no
    
    # AllowTcpForwarding: 禁止 TCP 转发（安全）
...
log ""
log "性能优化："
log "  ✓ SSH 缓冲区: 32MB (默认 64KB，提升 500x)"
